...
public class ByteCodeClassTransformer extends AbstractClassTransformer implements Opcodes {
	...
	private byte[] generate(Class<?> classToProxy, String proxyName, Method[] methods, Class<?>... interfaces) throws CreateProxyException {
		...
		String[] interfaceNames = Arrays.stream(interfaces).map(i -> Type.getType(i).getInternalName()).toArray(String[]::new);
	...
	private void initialize(ClassWriter cw, Type proxyType, Type superType) {
		GeneratorAdapter adapter = new GeneratorAdapter(ACC_PUBLIC, new org.objectweb.asm.commons.Method("<init>", Type.VOID_TYPE, new Type[] { INVOKER_TYPE }), null, null, cw);
		adapter.loadThis();
	...
	private Type[] getTypes(Class<?>... classes) {
		Type[] result = Arrays.stream(classes).map(Type::getType).toArray(Type[]::new);
		return result;
	}
